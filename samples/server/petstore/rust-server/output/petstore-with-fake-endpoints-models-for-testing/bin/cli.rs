...
enum Operation {
    ...
    AddPet {
        ...
        #[structopt(parse(try_from_str = parse_json))]
        body: swagger::OneOf2<models::AddPetApplicationJsonRequest,models::AddPetApplicationXmlRequest>,
    },
    /// Deletes a pet
    DeletePet {
        /// Pet id to delete
    ...
    UpdatePet {
        /// Pet object that needs to be added to the store
        #[structopt(parse(try_from_str = parse_json))]
        body: swagger::OneOf2<models::UpdatePetApplicationJsonRequest,models::UpdatePetApplicationXmlRequest>,
    },
...
async fn main() -> Result<()> {
    ...
    let client: Box<dyn ApiNoContext<ClientContext>> = if args.client_certificate.is_some() {
...
// May be unused if there are no DELETEs on this API
#[allow(dead_code)]
fn prompt(force: bool, text: &str) -> Result<()> {
    if force || Confirmation::new().with_text(text).interact()? {
        Ok(())
    } else {
        Err(anyhow!("Aborting"))
    }
